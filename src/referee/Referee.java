...
public class Referee {
	...
	public Referee(String command_to_execute_player1,
			String command_to_execute_player2, int board_height,
			int board_width, int N, int announceNameTimeLimit, int moveTimeLimit)
			throws IOException {
		...
		output1 = new BufferedWriter(new OutputStreamWriter(
				p1.getOutputStream()));
	...
	/**
	 * Once the players receive game information, the game starts immediately.
	 * Game information consists of 5 numbers [in this order]: 1.board height
	 * (#rows), 2.board width (#columns), 3.number of pieces to win (the N in
	 * Connect-N), 4.turn of the player (1 indicating 1st player, and 2
	 * indicating 2nd player), 5.the time limit to make a move in seconds.
	 * 
	 * @throws IOException
	 */

	public void run() throws IOException {
		String s = this.board.height + " " + this.board.width + " " + this.N
				+ " " + this.currentPlayer + " " + this.moveTimeLimit;
	...
	/**
	 * Referee read players' name from input. The players have 10 seconds to
	 * announce themselves by sending their name to the referee
	 */
	private void readPlayerName() throws IOException {
		System.out.println("Please enter two players user name. You have "
				+ this.announceNameTimeLimit + "s to enter.");
	...
	/**
	 * Referee keeps passing the opponent's move to the other player depending
	 * on the state of the game 1. [0,(COLS-1)] [0,1] indicating the oponent's
	 * move; 2. -1 indicating that the player won the game; 3. -2 indicating
	 * that the player lost the game; and 4. -3 indicating a tie.
	 */
	...
	/**
	 * Check current the move is valid. Invalid Moves can be: 1. Drop a disc on
	 * a column that is already completely filled. 2. Choosing an undefined
	 * column. 3. Try to poppout a disc of opponent's from the bottom of a
	 * column
	 * 
	 * @param column
	...
	private boolean isValidMove(int column, int operation) {
		if (operation == DROP) {
			...
			if ((this.board.canRemoveADiscFromBottom(column, currentPlayer) && (popOutCount[currentPlayer - 1] == 0)))
				return true;
			else
				return false;
		} else {
	...
	/**
	 * Place a disc from the top of the column or remove from bottom (see method
	 * comment)
	 * 
	 * @param column
	 *            The column in which the current move is going to make.
	...
	/**
	 * The game ends in any of the following scenarios: 1. One of the players
	 * gets N checkers in a row (horizontally, vertically or diagonally). 2. The
	 * board is full but none of the players managed to get N checkers in a row.
	 * 3. One player makes an illegal move, in this case the other player wins.
	 * 4. If one player fails to respond within the time limit, then the other
	 * player wins. This method only check 1 and 2 scenarios
	 * 
	...
	private boolean isEnd() {
		// if somebody connectN, he is the winner, else if the board is full,
		// game ends with tie
	...
	public static void main(String[] arg) throws IOException {
		...
		// String command_to_execute_player2=arg[1];
		// Referee ref=new
		// Referee(command_to_execute_player1,command_to_execute_player2);

		// String command_to_execute_player1="java -jar C:" + File.separator
		// +"courses2013fall" + File.separator +"testPlayer1.jar";
		// String command_to_execute_player2="java -jar C:" + File.separator
		// +"courses2013fall" + File.separator +"testPlayer2.jar";
		// Referee ref=new
		// Referee(command_to_execute_player1,command_to_execute_player2,6,7,3,10,10);
